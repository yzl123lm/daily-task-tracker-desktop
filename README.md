...
- **支持平台**
  - **Windows 10/11**：完整交付（NSIS 安装包 + 便携版 + `ship:latest-client` 一键安装）
  - **macOS 12+（Monterey 及以上）**：可本地开发与打包 DMG/ZIP（需在 Mac 上执行 `npm run build:mac`；GitHub Actions 也会自动构建）
- 可选：Python 3.12 + `.venv`（仅用于 `scripts/generate-app-icon.py` 图标生成）

> **说明**：Electron 无法在 Windows 上交叉编译 macOS 安装包；Mac 版请在 Mac 本机或 CI（`.github/workflows/build-desktop.yml`）中构建。加密 Excel 入库依赖 Windows + Microsoft Excel；Mac 上仍支持 xlsx 常规解析与知识库其余功能。
...
产物在 `dist/`（Windows：NSIS 安装包 + 便携版；macOS：`npm run build:mac` 生成 DMG + ZIP）。
...
npm run build:mac:dmg      # 仅 DMG
bash scripts/build-mac.sh  # 同上（便捷脚本）
```

未签名包首次打开若被拦截：系统设置 → 隐私与安全性 → 仍要打开。CI 产物见 GitHub Actions「Build desktop clients」。
...
| 命令 | 说明 |
|------|------|
| `npm run build:mac` | macOS DMG + ZIP（需在 Mac 上运行） |
| `npm run build:mac:dmg` | 仅 macOS DMG |
| `npm run build:mac:zip` | 仅 macOS ZIP |
| `npm run build:all` | 当前系统可用的 Win + Mac 目标（Mac 上可一次打双平台） |
